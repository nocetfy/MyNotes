...
<li>Huginn 抓取教程：<a href="https://newzone.top/posts/2018-10-07-huginn_scraping_any_website.html" target="_blank" rel="noopener noreferrer">RSS 进阶篇：Huginn - 真·为任意网页定制 RSS 源（PhantomJs 抓取）<ExternalLinkIcon/></a></li>
</ul>
...
<p>Huginn 任务有时会被卡住，后续任务都无法进行。原本使用官方 Docker 镜像，但重启容器无法解决任务卡住的问题。因此，改为手动部署 Huginn，定期使用重置命令防止任务卡住。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>部署步骤记录在下方，但还有 3 个<strong>待解决问题</strong>：</p>
<ul>
<li>
<p>定期导出数据库到本地。</p>
</li>
<li>
<p>测试任务卡住后，rake production:export 是否有效。</p>
</li>
<li>
<p>开机启动 Huginn 服务，需在 Docker 容器终端机中执行。runsvdir 在 SSH 中会报错 <code v-pre>supervise/lock: temporary failure</code>，但在终端机中运行正常。<code v-pre>production:export</code> 步骤会提示 <code v-pre>unable to open supervise/stat.new: file does not exist</code>，但此报错似乎不影响 Huginn 的运行，等有时间看看是否有相关报错。</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">sudo</span> <span class="token function">service</span> mysql restart
...
<p>部署环境：Ubuntu 18.04 的 Docker 镜像（同样适用于服务器）
安装参考：<a href="https://github.com/huginn/huginn/blob/master/doc/manual/installation.md" target="_blank" rel="noopener noreferrer">Manual Installation on Debian/Ubuntu<ExternalLinkIcon/></a>，<a href="https://github.com/huginn/huginn/wiki/Novice-setup-guide" target="_blank" rel="noopener noreferrer">Novice-setup-guide<ExternalLinkIcon/></a></p>
<p>Huginn 部署步骤：</p>
...
<span class="token function">sudo</span> <span class="token function">apt-get</span> <span class="token function">install</span> <span class="token parameter variable">-y</span> runit build-essential <span class="token function">git</span> zlib1g-dev libyaml-dev libssl-dev libgdbm-dev libreadline-dev libncurses5-dev libffi-dev <span class="token function">curl</span> openssh-server checkinstall libxml2-dev libxslt-dev libcurl4-openssl-dev libicu-dev <span class="token function">logrotate</span> python-docutils pkg-config cmake nodejs graphviz jq
...
<span class="token function">curl</span> <span class="token parameter variable">-L</span> --progress-bar https://cache.ruby-lang.org/pub/ruby/2.7/ruby-2.7.6.tar.bz2 <span class="token operator">|</span> <span class="token function">tar</span> xj
<span class="token builtin class-name">cd</span> ruby-2.7.6
...
<span class="token comment"># 逐行输入代码到数据库命令行 `mysql>`，需将 `$password` 替换为你要设置的密码</span>
...
<span class="token comment"># Clone Huginn repository</span>
<span class="token function">sudo</span> <span class="token parameter variable">-u</span> huginn <span class="token parameter variable">-H</span> <span class="token function">git</span> clone https://github.com/huginn/huginn.git <span class="token parameter variable">-b</span> master huginn
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>sudo -u huginn -H editor .env</code> 设置 huginn 环境依赖。编辑器为上面安装的 vim，<code v-pre>i</code> 在光标所在的位置插入，<code v-pre>esc</code> 退出编辑，<code v-pre>:qw</code> 保存并退出。</p>
...
gem <span class="token function">install</span> bundler:2.3.10
<span class="token comment"># Docker 环境中，时区容易丢失</span>
...
<span class="token comment"># 设置登陆账户密码，Create admin user and example agents using the default admin/password login</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>sudo -u huginn -H editor Procfile</code> 修改 huginn 设置。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>sv stop huginn-web-1</code> 的报错，使用 <code v-pre>foreman export runit -a huginn -l /home/huginn/huginn/log /etc/service</code>，<code v-pre>chown -R huginn:huginn /etc/service/huginn*</code>。<sup class="footnote-ref"><a href="#footnote2">[2]</a><a class="footnote-anchor" id="footnote-ref2" /></sup></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>以上完成了 Huginn 的所有部署。</p>
<h3 id="huginn-docker" tabindex="-1"><a class="header-anchor" href="#huginn-docker" aria-hidden="true">#</a> Huginn Docker</h3>
<p>Huginn <a href="https://github.com/huginn/huginn/tree/master/docker/multi-process" target="_blank" rel="noopener noreferrer">multi-process<ExternalLinkIcon/></a> 镜像基于 Ubuntu 18.04，没有 root 权限。如果不导出卷，或者使用单独的数据库容器，则无法在不丢失数据的情况下更新 Huginn。</p>
...
<h4 id="正则重构" tabindex="-1"><a class="header-anchor" href="#正则重构" aria-hidden="true">#</a> 正则重构</h4>
<p>比如生成时间规则为 <code v-pre>&quot;created_at&quot;: &quot;{{created_at}}&quot;</code>，默认时间 <code v-pre>2022-07-06 21:09:51 +0800</code>，使用正则删除规则为
...
<p><code v-pre>Creates events</code> <code v-pre>Receives events</code> <code v-pre>Dry runs</code>
...
<p>The Attribute Difference Agent receives events and emits a new event with the difference or change of a specific attribute in comparison to the previous event received.
...
<p><code v-pre>Creates events</code> <code v-pre>Receives events</code></p>
<p>The Change Detector Agent receives a stream of events and emits a new event when a property of the received event changes.
...
<p><code v-pre>Creates events</code> <code v-pre>Receives events</code> <code v-pre>Consumes file pointer</code> <code v-pre>Dry runs </code>
...
<p>The <code v-pre>DktClusteringAgent</code> clusters the input document collection. The document collection first has to be converted to a set of vectors.
...
<p>Include the <code v-pre>dropbox-api</code> and <code v-pre>omniauth-dropbox</code> gems in your <code v-pre>Gemfile</code> and set <code v-pre>DROPBOX_OAUTH_KEY</code> and <code v-pre>DROPBOX_OAUTH_SECRET</code> in your environment to use Dropbox Agents.</p>
<hr>
<h3 id="email-agent-邮件触发器" tabindex="-1"><a class="header-anchor" href="#email-agent-邮件触发器" aria-hidden="true">#</a> Email Agent - 邮件触发器？</h3>
<p><code v-pre>Receives events</code></p>
<p>Email Agent 收到任何内容后，会立即发送邮件通知。</p>
<p>You can specify the email’s subject line by providing a <code v-pre>subject</code> option, which can contain <a href="https://github.com/huginn/huginn/wiki/Formatting-Events-using-Liquid" target="_blank" rel="noopener noreferrer">Liquid<ExternalLinkIcon/></a> formatting. E.g., you could provide <code v-pre>&quot;Huginn email&quot;</code> to set a simple subject, or <code v-pre>{{subject}}</code> to use the <code v-pre>subject</code> key from the incoming Event.</p>
<p>By default, the email body will contain an optional <code v-pre>headline</code>, followed by a listing of the Events’ keys.</p>
<p>You can customize the email body by including the optional <code v-pre>body</code> param. Like the <code v-pre>subject</code>, the <code v-pre>body</code> can be a simple message or a Liquid template. You could send only the Event's <code v-pre>some_text</code> field with a <code v-pre>body</code> set to <code v-pre>{{ some_text }}</code>. The body can contain simple HTML and will be sanitized. Note that when using <code v-pre>body</code>, it will be wrapped with <code v-pre>&lt;html&gt;</code> and <code v-pre>&lt;body&gt;</code> tags, so you do not need to add these yourself.</p>
...
<p>The <code v-pre>FremeExploreAgent</code> can retrieve description of a resource from a given endpoint. The endpoint can be SPARQL or Linked Data Fragments endpoint.
