<template><div><p>Github 上搭建 Jekyll 是最方便的，空间免费、流量免费、部署简单。但 Github 属于被墙状态，将博客部署在那，速度实在太慢。在玩了几天后，我开始在服务器上直接搭建 Jekyll 博客。</p>
<p>服务器搭建需要人工执行 <code v-pre>jekyll build</code>, 完全背离了最开始搭建博客的初衷-<strong>方便</strong>。之后结合了网络上多个自动化方案，选定入门成本最低的 <code v-pre>Github</code> -&gt; <code v-pre>Travis CI</code> -&gt; <code v-pre>Docker</code>-&gt; <code v-pre>VPS</code>。</p>
<h2 id="搭建思路" tabindex="-1"><a class="header-anchor" href="#搭建思路" aria-hidden="true">#</a> 搭建思路</h2>
<ul>
...
<h2 id="travis-ci-基本配置" tabindex="-1"><a class="header-anchor" href="#travis-ci-基本配置" aria-hidden="true">#</a> Travis CI 基本配置</h2>
...
<h2 id="静态文件更新" tabindex="-1"><a class="header-anchor" href="#静态文件更新" aria-hidden="true">#</a> 静态文件更新</h2>
<p>Travis CI push 静态文件到 Github 通过 Github 的 token 实现授权，push 代码如下</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>after_success:
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>$DEPLOY_TOKEN</code> 是从 Github 授权得到的，「setting」&gt;「Developer settings」&gt;「Personal access tokens」&gt;「Generate new token」, 然后给于相应权限即可，<code v-pre>admin:public_key, admin:repo_hook, repo</code>。</p>
<figure><img src="http://tc.seoipo.com/20180504153729.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>进入 Travis 的 repo 项目，「More options」&gt;「Settings」&gt;「Environment Variables」, 新建一个变量<code v-pre>DEPLOY_TOKEN</code>，把 Github 的授权 token 保存在里面。</p>
<figure><img src="http://tc.seoipo.com/20180504154229.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>Travis CI 提供了存放加密文件的方式，参考 <a href="https://docs.travis-ci.com/user/encrypting-files/" target="_blank" rel="noopener noreferrer">官方文档<ExternalLinkIcon/></a>。</p>
<h2 id="dockerfiles-设置" tabindex="-1"><a class="header-anchor" href="#dockerfiles-设置" aria-hidden="true">#</a> Dockerfiles 设置</h2>
<p>在 Github 中新建一个 repository，可以命名为 <code v-pre>dockerfiles</code>, 专门用来存储 Docker 镜像的设置文件。在 <code v-pre>dockerfiles</code> 新建文件夹 <code v-pre>jekyll</code> ，并在 <code v-pre>jekyll</code> 中新建文件 <code v-pre>Dockerfile</code>, 放入以下代码：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>FROM nginx:1.13.9-alpine
...
<p>样例 Dockerfile: <a href="https://github.com/mritd/dockerfile/tree/master/mritd" target="_blank" rel="noopener noreferrer">https://github.com/mritd/dockerfile/tree/master/mritd<ExternalLinkIcon/></a></p>
<h2 id="docker-镜像设置" tabindex="-1"><a class="header-anchor" href="#docker-镜像设置" aria-hidden="true">#</a> Docker 镜像设置</h2>
<p>注册并登录 <a href="https://hub.docker.com" target="_blank" rel="noopener noreferrer">Docker Hub<ExternalLinkIcon/></a>，点击 「Create」&gt;「Create Automated Build」&gt;「Create Auto-build Github」, 选择之前新建的 <code v-pre>dockerfiles</code> repository。</p>
<p>建立 Automated Build 镜像后，进入 <code v-pre>Build Seeting</code>, 点击 Trigger，建立第一个 Docker 镜像。</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> pull rockben/jekyll
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>--name=jekyll_blog 中的 <code v-pre>jekyll_blog</code> 是对容器的命名，方便后续操作。</p>
<p>-d 让容器在后台运行。</p>
<p>-p 映射端口：80 是容器内对应的端口，39100 是主机端口，也就是最终用户访问的端口，本端口可以自由选择。</p>
<p>--privileged=true 关闭安全权限，否则你容器操作文件夹没有权限。</p>
<p>--<code v-pre>rockben/jekyll:latest</code> 是容器名称，可省略 <code v-pre>:latest</code>。</p>
<p>运行容器后，访问 <code v-pre>seoipo.com:39100</code> 就可以看到镜像网页。如果每次用端口访问，可以在域名 DNS 中设置显性 URL，将二级域名 <code v-pre>blog.seoipo.com</code> 指向 <code v-pre>seoipo.com:39100</code></p>
<h3 id="docker-扩展阅读" tabindex="-1"><a class="header-anchor" href="#docker-扩展阅读" aria-hidden="true">#</a> Docker 扩展阅读</h3>
<p><strong>Docker 命令符</strong>：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> <span class="token function">ps</span> // 查看所有正在运行容器
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong><code v-pre>docker run</code> 进阶设置</strong></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">--name</span><span class="token operator">=</span>jekyll_blog <span class="token parameter variable">-d</span> <span class="token parameter variable">-p</span> <span class="token number">39100</span>:80 <span class="token parameter variable">-v</span> /www/wwwroot/jekyll:/jekyll <span class="token parameter variable">--privileged</span><span class="token operator">=</span>true rockben/jekyll:latest /bin/bash
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>-v 挂载目录/root/software 本地目录 /software 容器目录，在创建前容器是没有 software 目录的，docker 容器会自己创建
--<code v-pre>/bin/bash</code> 这是 CMD 命令行，可不填</p>
<h2 id="ssh-免密码登录" tabindex="-1"><a class="header-anchor" href="#ssh-免密码登录" aria-hidden="true">#</a> SSH 免密码登录</h2>
<p>Travis 不能利用用户名和密码登陆，我们只有利用<strong>SSH 免密登陆</strong>服务器，更新并重启 Docker 容器。</p>
<p><strong>1、生成公钥/私钥对</strong></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token builtin class-name">cd</span> ~/.ssh  <span class="token comment"># 切换 .ssh 目录，目录的第一个字符如果是 `.` 表示改文件夹是隐藏文件夹</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2、将生成的公钥添加为受信列表</strong></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token builtin class-name">cd</span> ~/.ssh  <span class="token comment"># 切换.ssh 目录</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>3、在.ssh 目录下新增配置文件 config</strong></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token builtin class-name">cd</span> ~/.ssh  <span class="token comment"># 切换 .ssh 目录</span>
<span class="token function">vim</span> config  <span class="token comment">#新建并打开目录</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>点击 <code v-pre>i</code> 进入编辑状态，输入下列代码。完毕后，点击 <code v-pre>Esc</code> 退出编辑状况，然后输入 <code v-pre>:wq!</code> 强制保存后离开文件</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>Host <span class="token builtin class-name">test</span>
...
<p><code v-pre>gem install travis</code></p>
<p><strong>5、服务器创建空白<code v-pre>.travis.yml</code>文件</strong></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">mkdir</span> /home/travis <span class="token comment">#新建 travis 文件夹</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>6、服务器登录 Travis，添加加密的私钥至代码仓库</strong></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token builtin class-name">cd</span> /home/travis  <span class="token comment">#进入 .travis.yml 所在文件夹</span>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>Make sure to <span class="token function">add</span> id_rsa.enc to the <span class="token function">git</span> repository.
...
<li>
<p><strong>将新生成的 <code v-pre>id_rsa.enc</code> 文件上传到 Github 源文件 repository 中</strong></p>
</li>
<li>
<p>将 <code v-pre>.travis.yml</code> 中的 <code v-pre>openssl aes-256-cbc -K $encrypted_5c280379e96c_key -iv $encrypted_5c280379e96c_iv -in id_rsa.enc -out ~/.ssh/id_rsa -d</code> 放入最终的 <code v-pre>.travis.yml</code> 文件中。</p>
<figure><img src="http://tc.seoipo.com/20180504184508.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
</li>
</ul>
<h2 id="travis-yml-配置" tabindex="-1"><a class="header-anchor" href="#travis-yml-配置" aria-hidden="true">#</a> travis.yml 配置</h2>
<p>当项目内存在 <code v-pre>.travis.yml</code> 文件时，Travis CI 会按照其定义完成自动 build 过程，所以开启了上述配置以后还要在 Github 的 Jekyll 源文件项目下创建 <code v-pre>.travis.yml</code> 配置文件。</p>
<figure><img src="http://tc.seoipo.com/20180504141827.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p><code v-pre>.travis.yml</code> 配置文件内容样例如下：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>language: ruby
rvm:
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>具体 .travis.yml 配置，请参考 <a href="https://docs.travis-ci.com/" target="_blank" rel="noopener noreferrer">官方文档<ExternalLinkIcon/></a>。</p>
<p>参考资料&amp;引用：</p>
<ul>
<li>
<p><a href="https://github.com/Huxpro/huxpro.github.io" target="_blank" rel="noopener noreferrer">Jekyll 模板 hux blog<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="https://zhuanlan.zhihu.com/p/25066056" target="_blank" rel="noopener noreferrer">一点都不高大上，手把手教你使用 Travis CI 实现持续部署<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="https://mritd.me/2017/02/25/jekyll-blog-+-travis-ci-auto-deploy/" target="_blank" rel="noopener noreferrer">Jekyll + Travis CI 自动化部署博客<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="https://juejin.im/post/5a9e1a5751882555712bd8e1" target="_blank" rel="noopener noreferrer">Travis-CI 自动化测试并部署至自己的 CentOS 服务器<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="https://segmentfault.com/a/1190000011218410" target="_blank" rel="noopener noreferrer">Travis CI 系列：自动化部署博客<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="https://juejin.im/post/5a2941ad6fb9a045030ffc95" target="_blank" rel="noopener noreferrer">SSH 免密登录远程服务器<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="http://www.cnblogs.com/scofi/p/6617394.html" target="_blank" rel="noopener noreferrer">SSH 公钥登录原理<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="https://blog.csdn.net/yinweitao12/article/details/73165914" target="_blank" rel="noopener noreferrer">如何将 dockerhub 与 github 关联<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="http://www.cnblogs.com/YasinXiao/p/7736075.html" target="_blank" rel="noopener noreferrer">docker 启动，端口映射，挂载本地目录<ExternalLinkIcon/></a></p>
</li>
<li>
<p><a href="https://yeasy.gitbooks.io/docker_practice/" target="_blank" rel="noopener noreferrer">Docker — 从入门到实践<ExternalLinkIcon/></a></p>
</li>
</ul>
</div></template>
