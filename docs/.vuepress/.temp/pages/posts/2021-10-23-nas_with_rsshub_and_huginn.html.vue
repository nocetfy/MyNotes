<template><div><p>作为重度 RSS 用户，90% 的讯息来自 rss。RSSHub 生成主流媒体 rss，Huginn 定制个性化 rss，TinyTinyRSS 过滤 rss，这帮我<strong>从 3 小时的阅读时间节省到只需 1 小时</strong>。</p>
...
<h2 id="事前准备" tabindex="-1"><a class="header-anchor" href="#事前准备" aria-hidden="true">#</a> 事前准备</h2>
<ul>
<li>NAS</li>
<li>域名 (子域名既可)</li>
<li>带动态 DNS 的路由器 (推荐 openwrt 软路由)</li>
</ul>
<h2 id="docker-镜像安装" tabindex="-1"><a class="header-anchor" href="#docker-镜像安装" aria-hidden="true">#</a> Docker 镜像安装</h2>
<ol>
<li>
...
<h2 id="域名绑定" tabindex="-1"><a class="header-anchor" href="#域名绑定" aria-hidden="true">#</a> 域名绑定</h2>
<ol>
<li>
...
<ul>
<li>
<p>cloudflare 动态 DNS 配置 (<a href="https://p3terx.com/archives/openwrt-cloudflare-ddns.html" target="_blank" rel="noopener noreferrer">教程<ExternalLinkIcon/></a>)：系统－软件包，搜索「cloudflare」，安装 ddns-scripts_cloudflare.com-v4，然后重启路由器。</p>
</li>
<li>
<p>服务－动态 DNS，cloudflare 登录密码为 <a href="https://dash.cloudflare.com/profile/api-tokens" target="_blank" rel="noopener noreferrer">cloudflare API<ExternalLinkIcon/></a> 中的 Glodbal API Key。阿里云用户可在 RAM 访问控制中创建专门的 AccessKey。</p>
...
<h2 id="rss-转码" tabindex="-1"><a class="header-anchor" href="#rss-转码" aria-hidden="true">#</a> RSS 转码</h2>
<p>运营商禁用了家用宽带的 80 和 443 端口。这导致 NAS 的服务链接无法隐藏端口，如 <code v-pre>home.xxx.com:34567</code>。同时，主流 RSS 阅读器大多不支持配置端口链接，这导致在 NAS 上部署的 rss 源无法直接被读取。比如 Tiny Tiny RSS 订阅 NAS rss 源时不断报错，无法读取带端口的 rss 链接和 https 的 rss 源。</p>
<p>为了顺利获取 NAS 的 rss 源，我用 php 做了条转录链接，参考样例：<code v-pre>http://xxx.com/rss.php?type=yyy</code>。这个 php 文件聚合了所有的 rss 源，用链接参数作区分。虽然 php 转录需要部署在另一台服务器，但比起节省高配服务器的费用，还是值得的。对了，php 转录链接不吃配置，如果你已经有了一台，可以像我一样直接部署在工作服务器上。</p>
<div class="language-php line-numbers-mode" data-ext="php"><pre v-pre class="language-php"><code>## 网站目录新建 rss.php 文件，然后放入如下代码
