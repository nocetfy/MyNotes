...
<p>如果国内静态资源库没有你要的静态包，推荐用 <code v-pre>npm i</code> 命令将静态包下载到本地，并部署到阿里云/七牛云的国内服务器上，避免网页受 UNPKG 和 jsDelivr 屏蔽影响而出现偏差。</p>
<p>静态资源库：</p>
<ul>
<li><a href="https://cdn.bytedance.com/" target="_blank" rel="noopener noreferrer">字节 CDN<ExternalLinkIcon/></a>：国内测速表现最佳，缓存过期时间最长设置一年。</li>
<li><a href="https://www.staticfile.org/" target="_blank" rel="noopener noreferrer">Staticfile CDN<ExternalLinkIcon/></a>：CDN 加速由七牛云提供。</li>
</ul>
...
<p>第三方托管过于不稳定，最终我转为自托管 oss.newzone.top。</p>
...
<li>Vercel：推荐使用 GitHub 账户登录，有可能需要绑定国外手机号（不支持 Voice）。2022.08.26，「*.vercel.app」域名被 DNS 污染，需要绑定自定义域名。</li>
...
<ol>
<li>添加 CNAME 记录，将你的 IPFS 域名 (<a href="http://xxx.example.com" target="_blank" rel="noopener noreferrer">xxx.example.com<ExternalLinkIcon/></a>) 指向 <code v-pre>cloudflare-ipfs.com</code>。</li>
...
<p>如果你拥有服务器所有权限，可以使用 <a href="https://github.com/SamKirkland/web-deploy" target="_blank" rel="noopener noreferrer">web-deploy<ExternalLinkIcon/></a> 以 SSH 同步方式发布页面。但与 FTP 同步方式相比，是否安全性、速度、时间有区别，我还没测试过，仅做记录，</p>
<h3 id="同步到-oss" tabindex="-1"><a class="header-anchor" href="#同步到-oss" aria-hidden="true">#</a> 同步到 oss</h3>
